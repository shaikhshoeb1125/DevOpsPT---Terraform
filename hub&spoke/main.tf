...
resource "azurerm_firewall_network_rule_collection" "spoke_to_spoke" {
  ...
  rule {
    name = "dev-to-uat"
    source_addresses = [
      "10.1.0.0/16" # Dev spoke CIDR
    ]
    destination_addresses = [
      "10.2.0.0/16" # UAT spoke CIDR
    ]
    destination_ports = ["*"]
    protocols         = ["Any"]
  }
...
# 2. Allow spoke-to-onprem communication
resource "azurerm_firewall_network_rule_collection" "spoke_to_onprem" {
  ...
  rule {
    ...
    source_addresses = [
      "10.1.0.0/16", # Dev spoke
      "10.2.0.0/16", # UAT spoke
    ]
    destination_addresses = [
      "10.10.0.0/16" # On-premises network
    ]
    destination_ports = ["*"]
    protocols         = ["Any"]
  }
...
resource "azurerm_firewall_network_rule_collection" "internet_access_explicit" {
  ...
  # Allow internet (excluding RFC 1918 private ranges)
  rule {
...
# Separate rule collection with higher priority to block private IP ranges (optional)
resource "azurerm_firewall_network_rule_collection" "block_private_ranges" {
  name                = "block-private-ip-ranges"
  azure_firewall_name = azurerm_firewall.main.name
  resource_group_name = azurerm_resource_group.hub.name
  priority            = 120 # Higher priority (lower number) than internet allow rule
  action              = "Deny"
...
# # 3. Allow common infrastructure services (DNS, NTP, etc.)
# resource "azurerm_firewall_network_rule_collection" "infrastructure" {
...
# ============================================================================
# APPLICATION RULE COLLECTIONS (for Internet Sites access)
# ============================================================================

# 4. Allow outbound internet access for development
resource "azurerm_firewall_application_rule_collection" "spokes_internet" {
  ...
  rule {
    ...
    source_addresses = [
      "10.1.0.0/16", "10.2.0.0/16", "10.3.0.0/16" # Dev, UAT and Prod spoke
    ]
    target_fqdns = [
      "*.microsoft.com",
      "*.windows.net",
...
# # ============================================================================
# # NAT RULE COLLECTIONS (for inbound internet traffic if needed)
# # ============================================================================

# # 7. Example DNAT rules for inbound access
...
#     ]
#     destination_addresses = [
#       azurerm_public_ip.firewall.ip_address  # Firewall public IP
#     ]
#     destination_ports = ["443"]
#     protocols         = ["TCP"]
#     translated_address = "10.2.1.10"  # Example: UAT web server IP
#     translated_port    = "443"
#   }
# }
